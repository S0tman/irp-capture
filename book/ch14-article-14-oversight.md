...
The human oversight requirement creates a genuine design tension for high-volume AI deployments. A credit-scoring system processing 50,000 applications per day cannot have each application genuinely reviewed by a human in the same way a bespoke professional assessment can.
...
## How IRP Compliance Helps with Article 14

Article 14 compliance requires a record of human engagement — not just AI output. IRP Compliance is designed to capture precisely this: the human decision alongside the system's recommendation, in a structured record that survives audit.

The IRP pattern treats the human action as a first-class artefact. Every decision record includes not only what the AI recommended but what the human decided to do about it, when, and under what authorisation. This creates an audit trail that addresses the core Article 14 audit question: "Was a human actually in control?" — with a documented answer for every decision, rather than a general assertion that a review process exists.

The IRP Compliance Assessment includes questions on your oversight design (Questions 10–13) that map directly to Article 14's four dimensions: comprehension, observation, intervention, and halt authority.
