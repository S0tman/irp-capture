...
1. **Deterministic snapshots.** Canonicalise the ledger (RFC 8785), compute a stable digest, verify offline. Proves: this supplied state has not changed since the snapshot.
2. **External timestamp anchoring.** Submit only the digest to an RFC 3161 timestamp authority, store the detached receipt. Proves: this digest existed no later than the witnessed time.
